...
ZelzalVP_cmd = (
    "𓆩 [𝗭𝗧𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/ZThon) 𓆪\n\n"
...
async def variable(event):
    ...
    if (
        ...
        input_str == "المغرب"
        or input_str == "موريتانيا"
        or input_str == "الدار البيضاء"
    ):
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedub.zed_cmd(
    pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي|البوت|الستارت|ستارت) ?(.*)"
)
async def _(malatha):
    if malatha.fwd_from:
        return
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedub.zed_cmd(
    pattern="اضف صوره (الحماية|الحمايه|الفحص|الوقتي|البوت|الستارت|ستارت) ?(.*)"
)
async def _(malatha):
    if malatha.fwd_from:
        return
